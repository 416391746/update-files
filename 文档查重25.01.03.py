...
class DocumentComparer:
    ...
    def compare_and_save_common_images(self, progress_queue,similarity_threshold, batch_size):
        try:
            ...
            with ThreadPoolExecutor(max_workers=max(os.cpu_count() or 1, 1)) as executor:
                for start in range(0, total_tasks, batch_size // 10):
                    ...
                    # 批次处理结束后，释放资源并进行垃圾回收
                    del batch_pairs  # 删除对批次任务列表的引用
                    del futures  # 删除对 Future 对象的引用
                    gc.collect()  # 手动触发垃圾回收
                    logging.info("Garbage collected after processing a batch of images.")
            ...
            with ThreadPoolExecutor(max_workers=max((os.cpu_count() or 1) * 2, 1)) as executor:
                for start in range(0, total_tasks, batch_size//5):
                    ...
                    futures = [
                        ...
                        for (doc1, tables1), (doc2, tables2) in batch_pairs
                    ]

                    for future in as_completed(futures):
                        future.result()  # 确保异常被捕获
                        completed_tasks += 1
                        progress = int(90 * completed_tasks / total_tasks)
                        progress_queue.put(progress)
                    ...
                    del futures  # 删除对 Future 对象的引用
                    gc.collect()  # 手动触发垃圾回收
                    logging.info("Garbage collected after processing a batch of tables.")
    ...
    def compare_two_docs_static(documents, file_paths, paragraph_positions, i, j, batch_size=None,min_chars=30, filter_chars='', output_path='', matches_all_dict=None, common_matches_list=None, lock=None):
        ...
        for start in range(0, total_tasks, batch_size):
            ...
            with ThreadPoolExecutor(max_workers=max(os.cpu_count() or 1, 1)) as executor:
                futures = {
                    executor.submit(
                        DocumentComparer.process_paragraphs,
                        k, l, doc1, doc2, positions1, positions2,
                        min_chars, filter_chars
                    ): (k, l) for k, l in batch_tasks
                }
            ...
            logging.info(
                f"Batch completed at task {start // batch_size}. Current memory usage: {psutil.Process().memory_info().rss / 1024 / 1024:.2f} MB")
    ...
    def compare_documents(self, progress_queue,**kwargs):
        ...
        try:
            ...
            with self.lock:
                ...
                progress_queue.put(85)  # 更新进度条到85%
            logging.info("Image comparison completed.")
            ...
            with self.lock:
                ...
                progress_queue.put(100)  # 确保进度条更新到100%
            logging.info("Table comparison completed.")
...
class DocumentApp:
    def __init__(self, root):
        ...
        # 初始化默认值
        self.min_chars = tk.IntVar(value=30)
        ...
        self.batch_size = tk.IntVar(value=100)  # 默认批处理大小
    ...
    def open_settings(self):
        ...
        tooltips = {
            ...
            "min_chars": "文本连续最字符串值，一般30，数值越小，谨慎度越高，相应地比对效率越低",
            "batch_size": "每次处理的任务数量，默认100，数值越大，内存占用可能越高",
