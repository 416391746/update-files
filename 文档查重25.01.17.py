...
def download_and_replace(update_url_placeholder="[UPDATE_URL]", app_name="文档查重.exe", doc_name="操作手册.docx"):
    # 创建一个简单的 session 对象
    ...
    try:
        ...
        new_app_path = os.path.join(current_script_dir, app_name)
        ...
        # 尝试下载并替换本地的 version.txt 文件
        try:
            version_response = session.get(f"{update_url_placeholder}/version（勿删）.txt", timeout=60)
            ...
            logging.info("无法从远程服务器更新 version（勿删）.txt。使用本地文件。")
            if not os.path.exists(version_txt_path):
                logging.info("本地的version（勿删）.txt 不存在且无法从远程更新。")
                return False
        ...
        # 分块下载新版本 exe 文件
        try:
            response = session.get(f"{update_url_placeholder}/{app_name}", stream=True, timeout=None)
            response.raise_for_status()
            with open(new_app_path, 'wb') as f:
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        f.write(chunk)
            logging.info("成功下载新的应用程序文件。")
        except requests.exceptions.RequestException as e:
        ...
        try:
            ...
            size_limit = 500 * 1024 * 1024  # 500 MB，如果下载下来的exe文件小于500MB，就停止更新
            if file_size < size_limit:
                logging.error(f"下载的文件大小 ({file_size / (1024*1024):.2f} MB) 小于500 MB的阈值。更新已中止。")
                return False
            else:
        ...
        if os.path.exists(current_script_path):
            ...
            logging.info("已删除旧的应用程序文件。")
        else:
            logging.warning("未找到旧的应用程序文件。")
...
def perform_update_check_and_install(current_version, update_url, root=None):
    ...
    if has_update:
        if download_and_replace(update_url):
...
def main():
    ...
    # 确保 version.txt 文件存在
    current_version = "25.01.17"  # 替换为实际的当前版本号
