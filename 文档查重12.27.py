...
class DocumentComparer:
    ...
    def compare_two_docs(self, i, j):
        ...
        # 创建线程池
        with ThreadPoolExecutor(max_workers=max(os.cpu_count() or 1, 1)) as executor:  # 可根据需要调整最大工作线程数
            futures = {
        ...
        try:
            ...
            for i in range(len(self.file_paths)):
                for j in range(i + 1, len(self.file_paths)):
                    matches, i, j = self.compare_two_docs(i, j)
                    logging.debug(f"Comparing documents {i} and {j}, found {len(matches)} matches")
                    if matches:
            ...
            # 图片比较任务
            self.compare_and_save_common_images(progress_queue, self.similarity_threshold)
            with self.lock:
            ...
            # 表格比较任务
            self.compare_and_save_common_tables(progress_queue, self.tolerance)
            with self.lock:
